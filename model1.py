...
class TransformerClassifier(nn.Module):
    def __init__(self, encoder: Encoder, src_embed: InputEmbeddings, src_pos: PositionalEncoding, num_classes: int, d_model: int) -> None:
        super().__init__()
    ...
    def forward(self, x, mask):
        ...
        # Global Average Pooling: Mean across the sequence length
        x = x.mean(dim=1)
        return self.classifier(x)

def build_transformer_classifier(vocab_size, seq_len, num_classes, d_model=512, N=6, h=8, dropout=0.1, d_ff=2048):
    ...
    model = TransformerClassifier(encoder, embed, pos, num_classes, d_model)
